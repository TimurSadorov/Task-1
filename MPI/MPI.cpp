...
void solveThomas(const double* F,
    ...
    vector<double> A(length), B(length), C(length);
    ...
    vector<double> alpha(length), beta(length);
...
int main(int argc, char* argv[]) {
    ...
    if (processesCount != size) {
        if (rank == 0) {
            std::cout << "Process count should be " << size << std::endl;
        }
        MPI_Finalize();
        return 0;
    }
    ...
    if (rank == 0) {
        ...
        for (int t = 0; t < iterations; ++t) {
            ...
            if (t % 100 == 0) {
                ...
                string num = "" + std::to_string(t);
                string file_name = "array" + num + ".csv";
                // Создаем/открываем файл для записи
                std::ofstream file(file_name);

                for (int i = 1; i < size - 1; ++i) {
                    for (int j = 1; j < size - 1; ++j) {
                        file << temperature[i * size + j];
                        if (j != size - 2) {
                            file << ";";
                        }
                    }
